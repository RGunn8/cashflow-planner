...
// Called by app as: POST https://<project-ref>.functions.supabase.co/text/parse
...
Deno.serve(async (req) => {
  ...
  // Expect /parse
  if (url.pathname.endsWith('/parse') === false) {
  ...
  try {
    ...
    if (!text.trim()) return json({ error: 'Missing text', requestId }, { status: 400 });
